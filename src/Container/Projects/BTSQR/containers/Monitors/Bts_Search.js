...
const Search = () => {
        ...
        const table = () => {
            return ( <
                ...
                <
                thead className = "table-primary" > { tableHeader() } <
                /thead> { tableBody() } <
                /table>
            )
        }
        ...
        const tableHeader = () => {
            return ( <
                ...
                style = {
                    { marginLeft: '10px' } } > < Select func_setSelect = { func_setSelect }
                setState = { setStationCode }
                placeholder = { 'Station' }
                options = { stationList }
                /></div >
                <
                ...
                style = {
                    { width: "10%" } } >
                Terminal Code <
                div className = "align-self-center" > < input className = "form-control align-self-center search p17-5"
                type = "text"
                name = "terminalId"
                style = {
                ...
                onChange = {
                    (e) => setTerminalId(e.target.value) }
                /></div >
                <
                /th> <
                th className = "p17-5"
                ...
                style = {
                    { width: "15%" } } >
                Create Date <
                Date_range_picker func_setSelect = { func_setSelectDate }
            ...
            if (QRgens.length > 0) {
                ...
                QRgens.forEach((detail, index) => {
                    tableBody.push( <
                        ...
                        td className = "p17-5" > { detail.amount } < /td> { detail.confirm_datetime != null ? < td className = "p17-5"
                            style = {
                                { color: "Green" } } > Payment < /td> : <td className="p17-5" style={{ color: "Red" }}>No Payment</td > } <
        ...
        const paginations = () => {
                ...
                arrpage.push( < button className = "btn btn-outline-light text-primary"
                        style = {
                            { display: currentpage > 1 ? 'block' : 'none' } }
                        key = 'first'
                        value = { 1 }
                        onClick = { jumpPage } > { 1 } < /button>)
                        arrpage.push( < button className = "btn btn-outline-light text-primary"
                            style = {
                                { display: startPage > 2 ? 'block' : 'none' } }
                            key = "..Previous"
                            value = { currentpage - 1 }
                            onClick = { jumpPage } > ... < /button>)
                            for (let i = startPage; i <= endpage; i++) {
                                if (i === currentpage)
                                    arrpage.push( < button className = "btn btn-primary"
                                        key = { i }
                                        value = { i }
                                        onClick = { jumpPage } > { i } < /button>)
                                        else if (i > 1) {
                                            arrpage.push( < button className = "btn btn-outline-light text-primary"
                                                key = { i }
                                                value = { i }
                                        ...
                                        arrpage.push( < button className = "btn btn-outline-light text-primary"
                                            style = {
                                                { display: maxPage.length - currentpage > 5 ? 'block' : 'none' } }
                                            key = "..Next"
                                            value = { currentpage + 1 }
                                            onClick = { jumpPage } > ... < /button>)
                                            arrpage.push( < button className = "btn btn-outline-light text-primary"
                                                style = {
                                                    { display: endpage < maxPage.length ? 'block' : 'none' } }
                                                key = "end"
                                                value = { maxPage.length }
                                                onClick = { jumpPage } > { maxPage.length } < /button>)
                                                return (
                                                    arrpage
                                                )
                                            }
                                            ...
                                            return ( <
                                                ...
                                                style = {
                                                    { background: 'orange', padding: '1rem' } } >
                                                <
                                                ...
                                                div className = "mt-3 w-100" > {
                                                    ...
                                                    pagination &&
                                                        ...
                                                        <
                                                        button className = { disabled === true && currentpage === 1 ? "btn btn-outline-light text-muted disabled" : "btn btn-outline-light text-primary active" }
                                                    onClick = { prev } > Previous < /button> { paginations() } <
                                                        button className = { disabled === true && currentpage === maxPage.length ? "btn btn-outline-light text-muted disabled" : "btn btn-outline-light text-primary active" }
